...
| Method | Path | Description |
|--------|------|-------------|
| GET    | `/commerce/products` | Browse products (`?search`, `?categorySlug`, `?squareSlug` (products in a market), `?storeSlug` (a trader's goods), `?status`, `?sortBy`, `?minPrice`/`?maxPrice`, etc.) |
