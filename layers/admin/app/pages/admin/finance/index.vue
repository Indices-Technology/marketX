<template>
  <div class="mx-auto max-w-5xl space-y-6 p-6">
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-gray-900 dark:text-neutral-100">Finance</h1>
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
      ...
      >
        ...
        <p class="mt-2 text-lg font-bold text-gray-900 dark:text-neutral-100">
          <span v-if="financePending && !financeData" class="inline-block h-5 w-16 animate-pulse rounded bg-gray-100 dark:bg-neutral-800" />
          <span v-else>{{ t.value }}</span>
        </p>
        <p class="mt-0.5 text-[12px] text-gray-400 dark:text-neutral-500">{{ t.label }}</p>
      </div>
    </div>

    <!-- Payment status breakdown -->
    <div v-if="byStatus.length" class="flex flex-wrap items-center gap-2">
      <span class="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-neutral-500">
        All orders
      </span>
      <span
        v-for="s in byStatus"
    ...
    <div class="space-y-3">
      <div class="flex flex-wrap gap-2">
        <button
          ...
          @click="paymentFilter = f.value; offset = 0"
        >
          {{ f.label }}
        </button>
      ...
      <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
        <!-- Skeleton -->
        <div v-if="ordersPending && !ordersData" class="divide-y divide-gray-50 dark:divide-neutral-800">
          <div v-for="i in 6" :key="i" class="flex items-center gap-4 px-4 py-4">
            <div class="h-3 w-12 animate-pulse rounded bg-gray-100 dark:bg-neutral-800" />
            <div class="h-3 flex-1 animate-pulse rounded bg-gray-100 dark:bg-neutral-800" />
            <div class="h-3 w-16 animate-pulse rounded bg-gray-100 dark:bg-neutral-800" />
          </div>
        </div>

        <!-- Empty -->
        <div v-else-if="!orders.length" class="flex flex-col items-center justify-center py-16 text-center text-gray-400 dark:text-neutral-500">
          <Icon name="mdi:receipt-text-outline" size="30" class="mb-2 opacity-40" />
        ...
        <table v-else class="w-full text-[13px]">
          ...
          <tbody class="divide-y divide-gray-50 dark:divide-neutral-800">
            <tr v-for="o in orders" :key="o.id">
              <td class="px-4 py-3 font-medium text-gray-900 dark:text-neutral-100">#{{ o.id }}</td>
              <td class="px-4 py-3 text-gray-700 dark:text-neutral-300">{{ o.name || '—' }}</td>
              <td class="px-4 py-3 text-gray-500 dark:text-neutral-400">{{ o._count?.orderItem ?? 0 }}</td>
              <td class="px-4 py-3 font-semibold text-gray-900 dark:text-neutral-100">{{ formatNGN(o.totalAmount) }}</td>
              <td class="px-4 py-3 text-gray-500 capitalize dark:text-neutral-400">{{ o.paymentMethod }}</td>
              <td class="px-4 py-3">
                <span class="rounded-full px-2 py-0.5 text-[11px] font-semibold" :class="paymentClass(o.paymentStatus)">
                  {{ o.paymentStatus }}
                </span>
              </td>
              <td class="px-4 py-3 text-gray-500 dark:text-neutral-400">{{ formatDate(o.created_at) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const tiles = computed(() => {
  ...
  return [
    { label: `GMV · ${f.days}d`, value: formatNGN(f.gmv), icon: 'mdi:cash', color: 'green' },
    { label: `Orders · ${f.days}d`, value: (f.orders ?? 0).toLocaleString(), icon: 'mdi:receipt-text-outline', color: 'blue' },
    { label: 'Avg order value', value: formatNGN(f.aov), icon: 'mdi:chart-line', color: 'purple' },
    { label: `Shipping · ${f.days}d`, value: formatNGN(f.shipping), icon: 'mdi:truck-outline', color: 'amber' },
    { label: `Affiliate paid · ${f.days}d`, value: formatNGN(f.affiliatePaid), icon: 'mdi:account-cash-outline', color: 'rose' },
    { label: 'GMV · all time', value: formatNGN(f.allTimeGmv), icon: 'mdi:finance', color: 'gray' },
  ]
})
const SKELETON_TILES = [
  { label: 'GMV', value: '—', icon: 'mdi:cash', color: 'green' },
  { label: 'Orders', value: '—', icon: 'mdi:receipt-text-outline', color: 'blue' },
  { label: 'Avg order value', value: '—', icon: 'mdi:chart-line', color: 'purple' },
  { label: 'Shipping', value: '—', icon: 'mdi:truck-outline', color: 'amber' },
  { label: 'Affiliate paid', value: '—', icon: 'mdi:account-cash-outline', color: 'rose' },
  { label: 'GMV · all time', value: '—', icon: 'mdi:finance', color: 'gray' },
]

const byStatus = computed(() => {
  const m = fin.value?.byPaymentStatus ?? {}
  return Object.entries(m).map(([status, count]) => ({ status, count: count as number }))
})
...
const hasMore = computed(() => (ordersData.value as any)?.meta?.hasMore ?? false)
...
function paymentClass(status: string) {
  const map: Record<string, string> = {
    ...
    SHIPPING_PAID: 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400',
    PENDING: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
    UNPAID: 'bg-gray-100 text-gray-600 dark:bg-neutral-800 dark:text-neutral-400',
