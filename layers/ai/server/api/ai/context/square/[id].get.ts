// GET /api/ai/context/square/:id
// Returns full square context for RAG embedding.
...
export default defineEventHandler(async (event) => {
  ...
  if (!id) {
    throw createError({ statusCode: 400, statusMessage: 'Invalid square id' })
  }

  const data = await aiContextService.getSquare(id)
  if (!data) {
