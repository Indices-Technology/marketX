...
- Name: layers\shipping\server\api\shipping\__tests__\shipping-extended.spec.ts >> POST webhook/shippo — signature verification >> rejects request with a garbage signature
- Location: layers\shipping\server\api\shipping\__tests__\shipping-extended.spec.ts:152:3
...
> 160 |     expect(res.status()).toBe(401)
  ...
  249 |     })
  250 |     const order = (await orderRes.json()).data
  251 |     expect(order.status).toBe('DELIVERED') // unchanged — no downgrade
  252 |   })
  253 | 
  254 |   test.afterAll(async ({ request }) => {
  255 |     if (!ctx) return
  256 |     await archiveProduct(request, ctx.sellerToken, ctx.productId)
  257 |   })
  258 | })
  259 | 
```